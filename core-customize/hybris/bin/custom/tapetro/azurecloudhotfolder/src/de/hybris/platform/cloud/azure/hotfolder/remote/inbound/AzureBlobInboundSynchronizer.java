...
public class AzureBlobInboundSynchronizer implements InboundFileSynchronizer, BeanFactoryAware, InitializingBean, Closeable
{
    ...
    public AzureBlobInboundSynchronizer(final SessionFactory<CloudBlob> sessionFactory)
    {
        ...
        doSetMoveToRemoteDirectoryExpression(new LiteralExpression(client.getDirectoryDelimiter()));
    }
    ...
    private String generateLocalFileName(final String remoteFileName)
    {
        ...
        {
            final String localFileName = this.localFilenameGeneratorExpression.getValue(this.evaluationContext,
                remoteFileName, String.class);

            Assert.state(!StringUtils.isEmpty(localFileName),
                "'localFilenameGeneratorExpression' for '" + remoteFileName + "' was evaluated to null.");
